...
| `out/aurora-waves-v03-emerald-veil-in-motion.gif` | The animation: 96 frames, 9.6 seconds, about 5 MB. The same file is also at the top level. |
...
动画和静帧都放在本文件夹的 `out/` 里，点开就直接播放，不用再去别处找；顶层 `out/` 里另有一份完全相同的副本，主 README 的链接指向那一份。
